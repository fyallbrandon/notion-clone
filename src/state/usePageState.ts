...
export const usePageState = (initialState: Page) => {
    ...
    const addNode = (node: NodeData, index: number) => {
        setPage((draft) => {
            const newNodes = [...draft.nodes]; // Create a copy of the array
            newNodes.splice(index, 0, node); // Modify the copy
            draft.nodes = newNodes; // Set the modified copy back to the draft
        });
    }
